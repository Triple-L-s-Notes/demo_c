...
int main()
{
     ...
     * We can think of each of these steps are discrete operations, or functions.
     * We will define a function to perform each of these tasks, then here in main(),
     * link them together with our data.
     *
     ...
     *
     * The output of each function will be specified, but it's up to you on how to define them!
     * Each function's signature is stated in the utils.h file, with the definitions started in utils.c. You don't
     * have to change the function's signature/definition if you don't want to, but if you'd like
     * to solve the problem in a different way, you may.
     *
     * We are using a seperate C module called utils in order to demonstrate how a C program can be
     * built from multiple files, but it is not required. All your code could be written in a single
     * C file, but it is often helpful to organize code into header and module files as we have done.
     *
     * Start implementing below! Each step begins here in main(), and might jump over to utils if you need
     * to define a function housed there. Wherever you see "<code here>" there should be something to implement.
     *
     * However, if you come up with a clever solution to solve a problem or implement a solution, we'd love to see it.
     *
     * If you get stuck with a C concept or are looking for a pointer, head over to Piazza and feel free to post your code.
     * Instructors will be happy to point you in the right direction and students are encouraged to also provide support.
     *
     * Now: 3, 2, 1... Begin!
     */
    ...
    // We need to create a new buffer in memory (seperate from the input buffer) to hold our
    // packet. Since we only know the size of our packet since parsing it, we will have to
    ...
    */
    ...
    // Well, just to the commandline. We'll have to translate our byte array into a human readable format,
